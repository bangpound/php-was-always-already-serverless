...
# Compile PHP
docker build --build-arg PHP_VERSION=php-$PHP_VERSION -t php-fpm-build .

container=$(docker create php-fpm-build)

# Fetch the PHP binary in the container
mkdir sbin
docker -D cp $container:/php-src-php-$PHP_VERSION/sapi/fpm/php-fpm sbin/
...
#tar czf php-$PHP_VERSION.tar.gz php-fpm ext lib
#rm php-fpm
#rm -rf ext lib
